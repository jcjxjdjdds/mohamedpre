...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "T4_Mohamed":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "دارك")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطو دارك للشات  💎 .\n♪ مرحبا بك : @T4_Mohamed  💎 .**")
    dev = await get_dev(bot_username)
...
async def T4_Mohamed(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"مره واحد اسمو دارك  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝚂𝙾𝚄𝚁𝙲𝙴 𝙳𝙰𝚁𝙺](t.me/k_40_x) 💎 .
么  [𝒅𝒆𝒗 𝙳𝙰𝚁𝙺](t.me/T4_Mohamed) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["المطور دارك","دارك","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="T4_Mohamed")
...
async def sultan(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="k_40_x")
     name = user.first_name
     username = user.username 
     bio = user.bio
     user_id = user.id
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["T4_Mohamed"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["T4_Mohamed"]:
   ...
   if message.from_user.username == "T4_Mohamed":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور دارك  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @k_40_x  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @k_40_x  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @k_40_x  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["T4_Mohamed"]:
         await message.reply_text("**♪ رتبتك هي : المطور دارك  💎 .**")
         return
     if userr.username in ["T4_Mohamed"]:
